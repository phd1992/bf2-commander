...
- Arrival dismount (UX pass, per the commander's choice): when a jeep or APC carrying a squad reaches a flag it was ordered to ATTACK or DEFEND, the leader and passengers get out and 2 non-leader members stay aboard as driver and gunner so the vehicle keeps firing; tanks never auto-dismount. A plain MOVE keeps everyone aboard; `X` empties the vehicle. The same rule now serves the AI (its own dismount rule was removed). The APC follows the jeep rule by assumption.
- A MOUNT order no longer claims the vehicle; ownership is set when the first member boards, and a squad whose target vehicle filled up drops the order instead of walking to a full vehicle. The HUD only sends as many squads to a vehicle as its free seats allow.
